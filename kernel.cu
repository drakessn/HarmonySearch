...
void testBestParametersVec(Function*, functionType, string = "AckleyVec.csv");
...
int main()
{
	...
	Function* f = new Ackley(10);
	//searchBestParameters(f);
	//testBestParameters(f, Schwefel2, "schwefel2dgdgdfgh.csv");
	testBestParametersVec(f, Ackley10, "test.csv");
...
void testBestParametersVec(Function *f, functionType op, string filename)
{
	vector<Harmony> bests;
	Real sum = 0.0;
	cout << "Processing..." << endl;
#pragma omp parallel for
	for (auto i = 0; i < testSize; i++) {
